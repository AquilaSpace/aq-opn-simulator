...
function setupUIBindings() {
    ...
    // Turbulence model toggle and parameters
    const turbToggle = document.getElementById('turb-toggle');
    const turbParams = document.getElementById('turb-params');
    if (turbToggle) {
        turbToggle.addEventListener('change', () => {
            setTurbulenceEnabled(turbToggle.checked);
            if (turbParams) turbParams.style.display = turbToggle.checked ? '' : 'none';
            recomputeAllLinks();
        });
    }
    ...
    const turbTTSlider = document.getElementById('turb-tiptilt-slider');
    const turbTTInput = document.getElementById('turb-tiptilt-input');
    if (turbTTSlider && turbTTInput) {
        const updateTT = (v) => {
            setTurbulenceParams({ tiptiltGain: parseFloat(v) });
            recomputeAllLinks();
        };
        turbTTSlider.addEventListener('input', () => { turbTTInput.value = turbTTSlider.value; updateTT(turbTTSlider.value); });
        turbTTInput.addEventListener('input', () => { turbTTSlider.value = turbTTInput.value; updateTT(turbTTInput.value); });
    }

    // Turbulence Cn2 selector
    const turbCn2 = document.getElementById('turb-cn2');
    if (turbCn2) {
        turbCn2.addEventListener('change', () => {
            setTurbulenceParams({ Cn2: parseFloat(turbCn2.value) });
            recomputeAllLinks();
        });
    }
